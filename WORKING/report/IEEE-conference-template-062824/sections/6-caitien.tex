...
Hình~\ref{fig:gnn-prior-analysis} trình bày prior đồ thị được xây dựng từ thống kê đồng xuất hiện và tiếp xúc biên giữa các lớp nguyên liệu. Các cạnh mạnh thể hiện những cặp lớp thường xuất hiện cùng nhau hoặc có quan hệ không gian gần, chẳng hạn \texttt{lettuce}--\texttt{tomato}, \texttt{bread}--\texttt{chicken duck}, \texttt{rice}--\texttt{chicken duck}, \texttt{potato}--\texttt{steak}, \texttt{cake}--\texttt{strawberry} và \texttt{fish}--\texttt{ice cream}. Những quan hệ này cung cấp tín hiệu ngữ cảnh cho Relation GNN, giúp mô hình giảm nhầm lẫn giữa các vùng nguyên liệu có texture gần nhau hoặc thường cùng xuất hiện trong một món ăn.
...
Hình~\ref{fig:rtb-training-curves} cho thấy mô hình RTB/GNN hội tụ ổn định trong quá trình huấn luyện. Train loss giảm từ khoảng $12.1$ xuống $2.2$, trong khi validation loss giảm nhanh và duy trì thấp quanh khoảng $0.3$--$0.5$ ở cuối quá trình train. Các chỉ số validation cũng tăng đều theo epoch, trong đó \texttt{mIoU} và \texttt{mAcc} đạt tốt nhất ở epoch 141, còn \texttt{aAcc} đạt tốt nhất ở epoch 129. Điều này cho thấy mô hình không chỉ học tốt trên tập huấn luyện mà còn cải thiện ổn định trên tập đánh giá.
...
Hình~\ref{fig:rtb-presence-error} phân tích lỗi hiện diện lớp tại các checkpoint tiêu biểu. Ở epoch 0, mô hình gần như chưa nhận diện đúng sự hiện diện lớp, với FP $88.9\%$ và accuracy chỉ $3.7\%$. Tại checkpoint có \texttt{mIoU} tốt nhất, TP tăng lên $87.5\%$ và FN còn $12.5\%$; đặc biệt, checkpoint tốt nhất theo presence đạt $100\%$ TP. Kết quả này cho thấy graph reasoning giúp mô hình học tốt hơn quan hệ xuất hiện lớp trong ảnh, từ đó giảm dự đoán lan sang các lớp không có mặt.
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{img/caitien/plot/presence error.png}
    \caption{Phân tích lỗi hiện diện lớp của mô hình RTB/GNN tại các checkpoint tiêu biểu.}
