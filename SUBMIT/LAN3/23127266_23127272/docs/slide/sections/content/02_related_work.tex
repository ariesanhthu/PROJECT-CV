...
\begin{frame}{Nghiên cứu liên quan: dữ liệu và baseline}
  \begin{columns}
    \column{0.49\textwidth}
      ...
      \begin{itemize}
        \item 7.118 ảnh, 103 lớp, 42.097 mask
        \item DeepLabv3+ chỉ đạt \textbf{34.2 mIoU}
        \item ReLeM giúp SeTR tăng 41.3 $\to$ 43.9 mIoU
        \item Yêu cầu: nhạy với texture, boundary, class imbalance
      \end{itemize}
      \column{0.50\textwidth}
      \textbf{II-B. Lightweight segmentation}
      \begin{itemize}
        \item BiSeNet: 68.4\% mIoU @ 105 FPS
        \item Fast-SCNN: 68.0\% @ 123.5 FPS, 1.11M params
        \item PIDNet-S: 78.6\% @ 93.2 FPS
      \end{itemize}
...
\begin{frame}{Nghiên cứu liên quan: cơ chế tăng cường chi tiết}
  \begin{columns}
    \column{0.52\textwidth}
      ...
      \begin{itemize}
        \item Gated-SCNN: shape stream riêng
        \item PIDNet: 3 nhánh detail--context--boundary
        \item PointRend: tinh chỉnh tại điểm bất định
        \item Deep-TEN: texture như tín hiệu thống kê
      \end{itemize}
