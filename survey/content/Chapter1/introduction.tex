...
\section{Definition of Image Segmentation}
...
\section{Types of Image Segmentation}

Trong computer vision hiện đại, segmentation thường được chia thành ba loại chính:

\begin{itemize}
\item \textbf{Semantic segmentation}: Gán nhãn lớp cho mỗi pixel trong ảnh, nhưng không phân biệt các instance khác nhau của cùng một lớp. Ví dụ, tất cả pixel thuộc lớp ``car'' sẽ có cùng nhãn.
\item \textbf{Instance segmentation}: Không chỉ phân lớp pixel mà còn phân biệt các instance riêng biệt của cùng một đối tượng. Ví dụ, hai chiếc xe khác nhau được gán hai instance khác nhau.
\item \textbf{Panoptic segmentation}: Kết hợp semantic segmentation và instance segmentation, trong đó mỗi pixel được gán cả nhãn lớp và thông tin instance nếu đối tượng có thể phân biệt.
\end{itemize}
...
Survey này tập trung tổng hợp và phân tích các hướng tiếp cận chính trong semantic segmentation, đặc biệt là sự phát triển của các kiến trúc học sâu và xu hướng nghiên cứu gần đây.
