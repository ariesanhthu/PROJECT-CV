Ok. Mình sẽ **giữ nguyên mục đích và logic chính của bản 1-cell hiện tại**, nhưng **tách thành nhiều cell rõ ràng** để sau này bạn chỉnh từng đoạn dễ hơn.

Nguyên tắc mình giữ:

* vẫn là **BiSeNet V1 paper-style + Xception39-like**
* vẫn train trên `arischii05/cleaned-foodseg103`
* vẫn save checkpoint vào Drive
* vẫn giữ flow: config → dataset → transform → dataloader → model → optimizer/loss → metric → train/val → visualize
* **không bẻ sang framework khác**
* **không gom thêm một đống hàm mới không cần thiết**
  Mình chỉ tách lại từ code 1-cell bạn đang có. 
...
CFG = {
    ...
    # Save
    "SAVE_DIR": "/content/drive/MyDrive/[PROJECT][COMPUTER-VISION]/config1-base",
...
# CELL 6 — Download dataset from Kaggle
...
# CELL 8 — Read class mapping / infer num_classes
...
# CELL 11 — Optional trim + split train/val
...
# CELL 12 — Normalization constants
...
# CELL 18 — Model block: ConvBNReLU
...
# CELL 19 — Model block: SeparableConv2d
...
# CELL 28 — Build model / optimizer / loss / scaler
...
# CELL 30 — Resume checkpoint state
...
# CELL 31 — LR schedule + save checkpoint
...
## Cách chỉnh sau này cho dễ

Nếu sau này bạn muốn sửa theo từng mục, thì chỉnh đúng cell này:

* đổi dataset / root / checkpoint → **Cell 4, 6, 7, 8**
* đổi split → **Cell 11**
* đổi augmentation → **Cell 13, 14**
* đổi model → **Cell 18 → 27**
* đổi optimizer / loss / scaler → **Cell 28**
* đổi metric → **Cell 32**
* đổi train loop → **Cell 35, 36, 37**
* đổi visualization → **Cell 33, 34, 38**

## Một chỉnh nhỏ mình đã giữ khác với bản gốc

Mình để `NUM_WORKERS=0` trong config vì log trước của bạn đang dính lỗi `can only test a child process` trên Colab; phần còn lại vẫn giữ mục đích cũ. Kết quả smoke test trước cũng cho thấy pipeline chạy đúng nhưng model học rất yếu, nên việc tách cell ra để debug từng phần là hợp lý. 

Nếu muốn, tin nhắn tiếp theo mình sẽ viết luôn bản **overfit test 8 ảnh / 32 ảnh** cũng theo đúng format **chia cell y hệt kiểu này**.
